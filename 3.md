...
```bash
cat /etc/neutron/neutron.conf | grep -v '#'> neutron.conf.bkp
```

To view only commented lines
```vim
:g/^\(#\|$\)/d
```
...
```bash
cat /etc/neutron/plugins/ml2/ml2_conf.ini | grep -v '#'> ml2_conf.ini.bkp
...
```
    ...
    network_vlan_ranges = external,vlan:1:4096
    ...
...
```bash
cat /etc/nova/nova.conf | grep -v '#'> nova.conf.bkp2
vi /etc/nova/nova.conf
```
...
TODO: check if neutron.conf needs to be configured for keystone

```bash
cp /etc/neutron/plugins/ml2/linuxbridge_agent.ini linuxbridge_agent.ini.bkp
vi /etc/neutron/plugins/ml2/linuxbridge_agent.ini
```

    [linux_bridge]
    physical_interface_mappings = vlan:eth0,external:eth4
    physical_interface_mappings = provider:192.168.100.4
...
ToDo: Need to check this command
```bash
...
```
    ...
    for service in server plugin-linuxbridge-agent dhcp-agent metadata-agent l3-agent; do
...
ToDo: Operational OpenStack Identity service with appropriate configuration in the neutron.conf
