...
```
    ...
    [database]
    ...
    mysql+pymysql://keystone:password@ control/keystone
...
```bash
 ...
 --region RegionOne \ 
 identity public http:// control:5000/v2.0

 openstack endpoint create \
 --region RegionOne \
 identity internal http:// control:5000/v2.0

  openstack endpoint create \
 --region RegionOne \
  identity admin http:// control:35357/v2.0
...
```bash
 ...
 compute public http:// control:8774/v2/%\(tenant_id\)s
 
 openstack endpoint create --region RegionOne \   
 compute internal http:// control:8774/v2/%\(tenant_id\)s
 
 openstack endpoint create --region RegionOne \   
 compute admin http:// control:8774/v2/%\(tenant_id\)s
```
